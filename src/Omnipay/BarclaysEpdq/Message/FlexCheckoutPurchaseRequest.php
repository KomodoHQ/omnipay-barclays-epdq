 ...
 */
class FlexCheckoutPurchaseRequest extends AbstractRequest
{

    protected $liveEndpoint = 'https://payments.epdq.co.uk/Tokenization/HostedPage';
    protected $testEndpoint = 'https://mdepayments.epdq.co.uk/Tokenization/HostedPage';
    ...
    {
        return $this->setParameter('returnUrl', substr($value, 0, 200));
    }
    ...
    {
        $this->validate('clientId', 'language');
    ...
    protected function cleanParameters($data)
    {
        ...
        foreach ($data as $key => $value) {
            if (!is_null($value) && $value !== false && $value !== '') {
                $clean[strtoupper(str_replace('_', '.', $key))] = $value;
            }
        }
